...
async function writeTradeupsWithBackoff(env: CloudflareEnv, payload: TradeupCachePayload): Promise<void> {
...
export async function refreshProfitableTradeups(env: CloudflareEnv) {
    ...
    if (isLocked) {
        ...
        throw new Error(`Scan already in progress (started at ${lockTime})`);
    }
    ...
    try {
        ...
        if (!snapshot) {
            throw new Error("Price snapshot not found — run /api/prices/refresh first");
        }
        ...
        if (cached) {
            try {
                ...
                for (const contract of payload.contracts) {
                    contractMap.set(getContractKey(contract.inputs), contract);
                }
        ...
        const onUpdate = async (newlyFound: ProfitableContract[]) => {
            for (const contract of newlyFound) {
                contractMap.set(getContractKey(contract.inputs), contract);
            }
            ...
            try {
                await writeContractsToCacheSafely(env, mergedList, new Date().toISOString(), "incremental");
        ...
        for (const contract of runResults) {
            contractMap.set(getContractKey(contract.inputs), contract);
        }

        const finalMerged = [...contractMap.values()];
        const cachedAt = new Date().toISOString();
        const persistedCount = await writeContractsToCacheSafely(env, finalMerged, cachedAt, "final");
