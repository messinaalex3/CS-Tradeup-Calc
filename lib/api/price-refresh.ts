...
function applyItemToSnapshot(
    ...
    const existingMin = existing && typeof existing !== "number" && typeof existing.minPrice === "number"
        ? existing.minPrice
        : null;
...
async function fetchCs2CapSnapshotByPagination(
    ...
    while (true) {
        ...
        try {
            ...
            throw new Error(`CS2Cap paginated fetch failed after ${pages} page(s) and ${rows} row(s): ${detail}`);
        }
    }
...
export async function refreshPricesFromCs2Cap(
    env: CloudflareEnv,
    ...
    if (!apiKey) {
        ...
        throw new Error("CS2C_API_KEY not configured — set it as a Cloudflare secret");
    }
    ...
    try {
        ...
        throw new Error("Failed to fetch prices from CS2Cap");
    }
